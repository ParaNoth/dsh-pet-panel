/**
 * ⚠️⚠️ 这个文件**不是**运行中的代码，而且已落后于运行版本 ⚠️⚠️
 *
 * 本 fork 的宿主改动（会话面板的 `/sessions` 端点、workTextOf、`dsh-pet` 设置命名空间、
 * 终态清理的重排修复……）**只写在构建产物 `lib/index.js` 里，没有回写到本文件**：
 *   - 本文件仍是上游 v0.2.11 的状态（连 `/sessions` 端点都没有）；
 *   - **不要跑 `npm run bundle`** —— 那会用本文件重新生成 `lib/index.js`，把 fork 的功能全部覆盖掉；
 *   - 要改宿主逻辑，改 `lib/index.js`；
 *   - 想彻底解决（搬回这里、恢复可构建），见 CHANGELOG.fork.md 的「未做 / 待办」。
 *
 * ────────────────────────────────────────────────────────────────────────────
 *
